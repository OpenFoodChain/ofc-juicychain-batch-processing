...
from  openfood_lib_dev.openfood_env import KV1_ORG_POOL_WALLETS
...
from  openfood_lib_dev.openfood_env import DEV_IMPORT_API_RAW_REFRESCO_PATH

#from  openfood_lib_dev.openfood_env import TEST_GEN_WALLET_PASSPHRASE
#from  openfood_lib_dev.openfood_env import TEST_GEN_WALLET_ADDRESS
#from  openfood_lib_dev.openfood_env import TEST_GEN_WALLET_WIF
#from  openfood_lib_dev.openfood_env import TEST_GEN_WALLET_PUBKEY
...
def test_utxo_send():
    address = THIS_NODE_RADDRESS
    wif = THIS_NODE_WIF
    to_address = "RCJudMYKrfJZ3iFdvAM7h1reLRDtEooSve"
    utxos_json = json.loads(openfood.explorer_get_utxos(address))
    # send all amount from 2 utxos
    test = openfood.utxo_send(utxos_json[0:1], 100, to_address, wif, address)
    if {
        "txid"
    } <= set(test):
        assert True
...
def test_save_offline_wallet_sent():
    ...
    integrity_id = batch[0]["integrity_details"]["id"]
    test = openfood.save_offline_wallet_sent(integrity_id, {
        'PON': True,
        'JDS': True,
        'JDE': True,
        'PC': True,
...
def test_restart_offline_wallet_sent():
    ...
    batch = json.loads(get_batch)
    integrity_id = batch[0]["integrity_details"]["id"]
    test = openfood.restart_offline_wallet_sent(integrity_id)
    assert test

def test_str2int():
    # 12 alphanumeric string to 10 len
    test = openfood.str2int("example-1234", 10)
    assert len(str(test)) == 10
